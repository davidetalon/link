...
\begingroup
\thispagestyle{empty}
\AddToShipoutPicture*{\put(0,0){\includegraphics[scale=1.25]{network}}} % Image background
\centering
\vspace*{5cm}
\par\normalfont\fontsize{35}{35}\sffamily\selectfont
\textbf{Link application}\\
{\LARGE Send files over local network}\par % Book title
\vspace*{1cm}
{\Huge Davide Talon}\par % Author name
\endgroup
...
\noindent Progetto per il laboratorio di ingegneria informatica svolto tenuto dal professore Nicola Ferro. Svolto nel secondo semestre dell'anno accademico 2015 - 2016.\\ % License information
...
Quante volte ci è capitato di dover scambiare dei file tra computer vicini, come spesso capita in ufficio o in casa,
...
Questo progetto, infatti, nasce principalmente dalla propria passione per internet e le telecomunicazioni, un mondo che mi appassiona e entusiasma facendomi capire la potenza delle nuove tecnologie.
Tante volte in ufficio o in casa ci capita di dovere inviare files tra computer vicini e per farlo, normalmente ci serviamo di chiavette USB o dell'email.
Nasce così l'idea di utilizzare la connessione locale per lo scambio di file.
L'obiettivo di questo progetto è quello di poter permettere a un utente di scambiare file tra computer connessi alla stessa rete locale, in modo facile e veloce, senza dover ricorre alle classiche chiavette USB (in caso di necessità introvabili) e alla email il cui problema principale, purtroppo, è il limitato upload.


\section{Obiettivi}\index{Obiettivi}
Inizialmente l'obiettivo del progetto era quello di poter permettere a un utente medio di inviare, tramite l'applicazione, files da un computer a un altro, purchè essi fossero connessi alla stessa rete, indipendetemente dal sistema operativo utilizzato.
Questo significava creare una applicazione multi-piattaforma
Durante la fase di progettazione, però, ci si è resi conto che il funzionamento base dell'applicazione
...
\item Creare una applicazione multi-piattaforma indipendente dal sistema operativo
\item Semplicità nell'utilizzo dell'applicazione.
\end{itemize}
...
\section{Idea iniziale}\index{First ideas}
...
\section{Architettura}\index{Hypothesis}
